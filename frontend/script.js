    ...
    if (document.getElementById('map')) {
        ...
        const drawControl = new L.Control.Draw({
            ...
            draw: {
                polygon: {
                    ...
                    showArea: true,
                    shapeOptions: { color: '#646cff', weight: 3 }
                },
                rectangle: {
                    shapeOptions: { color: '#646cff', weight: 3 }
                },
        ...
        map.on(L.Draw.Event.EDITED, function (e) {
            if (currentDrawnLayer) {
                currentBounds = currentDrawnLayer.getBounds();
            }
        });
...
function renderResult(data) {
    ...
    if (currentAnalysisType === 'infrastructure' && resData.buildings_count !== undefined) {
        ...
        html += `
            <div class="stats-panel">
               ...
               <div style="display: grid; gap: 1rem;">
                   ...
                   <div style="display: flex; justify-content: space-between;">
                        <span style="color: var(--text-muted)">Количество зданий (видимых)</span>
                        <span style="font-weight: 600">${resData.buildings_count}</span>
                   </div>
               </div>
            </div>
        `;
    ...
    crops.forEach(c => {
        html += `
            <div style="min-width: 140px; background: rgba(0, 255, 136, 0.05); border: 1px solid rgba(0, 255, 136, 0.2); border-radius: 12px; padding: 12px; flex: 1;">
                <div style="font-size: 2rem; margin-bottom: 8px; text-align: center;">${c.icon}</div>
